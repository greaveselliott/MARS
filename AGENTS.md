...
12. **Version every source and target change unless an active source transition says otherwise.** In steady state, every non-release semantic commit in this repo is followed by `mars release notes --repo . --bump auto`, `mars release backfill-notes --repo . --check`, then a `release: notes X.Y.Z` commit. The owner-recorded launch transition is the current narrow source-only exception: T-071 through T-079, including resumed T-058 corrections, retain the `0.68.49` version floor and push validated semantic checkpoints without release-note commits. T-080 alone publishes signed `v0.69.0` and `v0.69.1`; evidence-only T-081 closeout retains `v0.69.1`, while any canary code or public-contract correction requires `v0.69.2`. Initialized target repos keep their generated versioning rule. Release-note commits are exempt and are ignored by the generator.
