...
12. **Version every source and target change unless an active source transition says otherwise.** In steady state, every non-release semantic commit in this repo is followed by `mars release notes --repo . --bump auto`, `mars release backfill-notes --repo . --check`, then a `release: notes X.Y.Z` commit. The owner-recorded F-018 transition is the current narrow source-only exception: T-065 through T-068 retain the `0.68.49` version floor and push validated semantic checkpoints without release-note commits. Initialized target repos keep their generated versioning rule. Release-note commits are exempt and are ignored by the generator.
...
F-018 transition, T-065 through T-068 retain the `0.68.49` version floor and
...
4. **Version after non-release commits except during an explicit source transition.** Follow rule 12. In steady state, run the release-note and backfill sequence immediately; during T-065 through T-068, retain `0.68.49` and do not generate a release-note commit.
