...
6. **BDD defines done.** Goals align the active plan, BDD feature contracts define feature completeness, and walking skeleton slices implement the next failing scenario through real E2E/integration evidence.
7. **Business logic is first-class BDD.** Every product rule, workflow branch, state transition, validation, permission, scoring/trust rule, routing rule, release classification, or user-visible outcome must be documented step by step in `docs/features/` before or alongside implementation.
8. **Quality is not a phase.** Tests are written alongside code. Every milestone has a quality gate.
