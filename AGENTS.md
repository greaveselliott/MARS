...
12. **Version every source and target change.** Every non-release semantic commit in this repo must be followed by `mars release notes --repo . --bump auto`, `mars release backfill-notes --repo . --check`, then a `release: notes X.Y.Z` commit before the task is considered done. If the backfill check reports legacy entries, run `mars release backfill-notes --repo .` and include those changelog corrections in the release-note commit. Initialized target repos receive the same rule through generated harness docs. Release-note commits are exempt and are ignored by the generator.
...
4. **Version after non-release commits.** Immediately after a semantic change commit, run `mars release notes --repo . --bump auto`, verify, run `mars release backfill-notes --repo . --check`, apply any required backfill, commit the release-note update, and push. Do not generate another version for the `release: notes` commit itself.
5. **Publish release assets locally first.** Once the release-note commit is pushed, tag `vX.Y.Z` at that commit, run `mars release publish-assets --repo . --version vX.Y.Z --upload auto`, and verify assets with `mars release verify-assets --dist dist/releases --version vX.Y.Z`. Missing GitHub mirror capability or failed asset verification is a blocker to record, not something to silently ignore.
