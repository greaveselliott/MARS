...
5. **No stale documentation.** All docs are live system artifacts. When writing or materially changing code, add or update the top-of-file `MarsDocSync` comment block listing associated docs, then update those docs in the same commit or record why no doc change was needed.
