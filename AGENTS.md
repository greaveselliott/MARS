...
14. **Use the repository-owned release producer.** MARS source release production is defined by the pinned GoReleaser/Syft configuration and the active release plan; generated target repositories choose and document their own producer rather than inheriting this Go-specific implementation. Produce and verify artifacts locally before remote publication, and record any production or verification blocker instead of treating transport success as completion.
...
After pushing the release-note commit, follow the repository-owned release plan and producer. For this source repo, use the pinned GoReleaser/Syft workflow only at the authority level permitted by the active plan; generated targets use their own documented producer. Require local artifact verification and independently verify any remote publication before advertising it. If production or verification is blocked, record that blocker before ending the task.
...
5. **Produce and verify release assets locally first.** Once the release-note commit is pushed, follow the repository-owned producer and artifact contract. MARS source uses the pinned GoReleaser/Syft workflow subject to the active plan; targets choose their own producer. Missing production capability or failed verification is a blocker to record, not something to silently ignore.
