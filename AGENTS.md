...
14. **Publish versioned GitHub releases when configured.** After a release-note commit is pushed, repos with authenticated GitHub release capability must publish or update a GitHub Release named `vX.Y.Z` using the generated changelog entry. For this source repo, push tag `vX.Y.Z` at the release-note commit and let the Release workflow attach binaries; then verify the release object with `gh release view vX.Y.Z`. If the workflow does not create the GitHub Release object, immediately create a notes-only release from the generated `CHANGELOG.md` entry with the existing tag, then record that assets are still blocked. If a notes-only GitHub Release already exists, run the Release workflow backfill for that tag or record the blocker. Always run `mars-harness release verify-assets --version vX.Y.Z` before claiming release work complete. If GitHub release publication or asset verification is unavailable, record the blocker explicitly instead of treating release work as complete.
...
5. **Publish GitHub release notes when configured.** Once the release-note commit is pushed, push tag `vX.Y.Z` at that commit so the Release workflow publishes the changelog entry and binary assets. Confirm `gh release view vX.Y.Z` succeeds; if the release object is missing, create a notes-only GitHub Release from the generated changelog entry for the existing tag, then continue to asset verification. If a notes-only release appears, run the Release workflow backfill for that tag. Verify assets with `mars-harness release verify-assets --version vX.Y.Z`. Missing GitHub release capability or failed asset verification is a blocker to record, not something to silently ignore.
