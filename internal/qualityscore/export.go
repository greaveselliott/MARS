...
func (ev *evidence) collect(ctx context.Context) error {
	...
	if err != nil {
		if isDatabaseEvidenceUnavailable(err) {
			ev.warnings = append(ev.warnings, fmt.Sprintf("SQLite score evidence unavailable at %s; score evidence is insufficient. Run `mars setup`, run `mars register --repo <path>`, or pass --db with a writable SQLite path.", ev.dbPath))
			return nil
		}
		return err
	}
...
func outcomeSignalTicketBody(proposal telemetry.ImprovementProposal, count scoring.OutcomeCount, window string) string {
	...
	fmt.Fprintf(&b, "`mars scores export` detected an intervention-debt outcome signal while refreshing `docs/QUALITY_SCORE.md`.\n\n")
...
func staleTicketSignalTicketBody(proposal telemetry.ImprovementProposal, tickets []ticketInfo, now time.Time) string {
	var b strings.Builder
	fmt.Fprintf(&b, "## Context\n\n")
	fmt.Fprintf(&b, "`mars scores export` detected stale in-progress ticket state while refreshing `docs/QUALITY_SCORE.md`.\n\n")
...
func regressionTicketBody(proposal telemetry.ImprovementProposal, sc scoring.Score) string {
	var b strings.Builder
	fmt.Fprintf(&b, "## Context\n\n")
	fmt.Fprintf(&b, "`mars scores export` detected a low role score while refreshing `docs/QUALITY_SCORE.md`.\n\n")
	fmt.Fprintf(&b, "## Evidence\n\n")
	fmt.Fprintf(&b, "- Role: %s\n", sc.Role)
	fmt.Fprintf(&b, "- Repo ID: %s\n", sc.RepoID)
...
func (ev evidence) render(grade string) string {
	...
	fmt.Fprintf(&b, "**Generated by:** `mars scores export --repo <path>`\n")
	if ev.repoID != "" {
		fmt.Fprintf(&b, "**Repo ID:** `%s`\n", ev.repoID)
	}
	if ev.dbPath != "" {
	...
	fmt.Fprintf(&b, "This file is the repo-visible quality artifact for MARS evidence. It is generated from the scoring database, telemetry, ticket state, and preserved manual notes so agents can inspect quality without treating the dashboard as the source of truth.\n\n")
	...
	fmt.Fprintf(&b, "- Refresh this artifact with `mars scores export --repo <path>`.\n")
