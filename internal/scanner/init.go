...
- After every non-release semantic commit, run ` + "`mars release notes --repo . --bump auto`" + `, verify ` + "`VERSION`" + ` and ` + "`CHANGELOG.md`" + `, ensure the generated entry explains ` + "`Impact`" + `, ` + "`Why`" + `, and ` + "`What Changed`" + ` before commit buckets, commit ` + "`release: notes X.Y.Z`" + `, and push ` + "`main`" + `. Then create immutable tag ` + "`vX.Y.Z`" + ` at that exact release-note commit and complete this repository's configured producer, publication, and independent verification gates before calling the work done. If a gate cannot pass, record ` + "`release_blocked`" + ` rather than completion. Do not generate another version for the release-note commit itself.
...
Harness-owned failures such as dispatch protocol failures, loop/max-turn failures, guardrail or tool-policy workflow failures, context or inference failures, manifest/tool-policy gaps, and unknown terminal failures remain raw local telemetry first. They do not create target backlog tickets by default. If the aggregate foundation telemetry mode named ` + "`anonymous`" + ` is explicitly enabled, the harness may derive an allowlisted minimized report for a configured collector. That telemetry payload excludes raw traces, prompts, repo paths, remotes, ticket text, command output, raw error messages, commit SHAs, usernames, file paths, and source content; its network transport is not anonymous, and other configured cloud or integration routes have separate data boundaries.
...
5. push to ` + "`main`" + `
6. immutable tag ` + "`vX.Y.Z`" + ` at that release-note commit, followed by this repository's configured production, publication, and independent verification gates; record ` + "`release_blocked`" + ` if any gate cannot pass
...
- Create exactly one ordinary feature ticket for the active plan's current
  failing scenario, or the earliest uncovered product scenario, when no
  completed product ticket with build/smoke evidence exists yet. Hand that first slice to
  Engineer before broader backlog expansion.
- After the first product ticket has completed build/smoke evidence, create or
  confirm the next one or two ordered scenario tickets when the project needs a
  small follow-on backlog. Use multiple ticket_create calls for independent
  slices, or one grouped ticket only when adjacent scenarios are genuinely the
  same bounded walking skeleton.
- When the active operating plan names a BDD feature or scenario schedule,
  ...
  historical contracts. Ensure the first-slice ticket covers the current
  failing scenario or earliest uncovered BDD scenario in that active feature
  schedule. Do not skip ` + "`F-001-S001`" + ` or its active-feature equivalent just
  because a later scenario looks more interesting.
...
- After creating or confirming that the first active-feature scenario is
  covered by an implementation ticket, commit the ticket changes, record
...
only for the current failing scenario before the first completed build/smoke
proof exists. After that proof exists, create the next one or two adjacent
product scenarios in the active feature schedule when follow-on backlog
expansion is needed. Do not decompose the same BDD scenario into several
independent backlog tickets.
...
6. BUILD VERIFICATION (mandatory before closing any ticket)
   ...
   e) For web projects, start the dev server briefly to verify it boots:
      ...
      Probe readiness with a separate command such as curl, then stop only the
      same-job tracked PID returned by ` + "`background:true`" + `. Direct shell-form
      ` + "`kill`" + `/` + "`pkill`" + `/` + "`killall`" + ` commands and untracked or cross-job PIDs are
      rejected; this helper policy is not containment against arbitrary host code.
      Never call shell_exec with a bare port token such as ` + "`:8080`" + `;
      ports are not commands.
...
stop only the same-job tracked PID with ` + "`shell_exec argv [\"kill\",\"<pid>\"]`" + `
after probes. Direct shell-form ` + "`kill`" + `/` + "`pkill`" + `/` + "`killall`" + ` commands and
untracked or cross-job PIDs remain blocked.
...
8. After the release-note commit is pushed, create immutable tag ` + "`vX.Y.Z`" + ` only at that commit, then complete this repository's approved producer, publication, and independent artifact-verification gates before calling the work done. MARS does not select a producer or generic release-asset verifier for target repositories. When GitHub Releases are used, independently require exact remote name/state/size/SHA-256 convergence before advertising availability. Treat partial or unverifiable state as ` + "`release_blocked`" + ` even after upload-process exit zero.
