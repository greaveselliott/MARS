...
func TestInit_success(t *testing.T) {
	...
	featureContract, err := os.ReadFile(filepath.Join(dir, "docs", "features", "F-001-delivery-operating-model.md"))
	require.NoError(t, err)
	...
	assert.Contains(t, string(featureContract), "F-001-S008: CLI Tool And Skill Synchronization")
	assert.Contains(t, string(featureContract), "F-001-S009: Remote Trunk Freshness And Immediate Publishing")
