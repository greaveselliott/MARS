...
func TestReadHarnessMetadataAcceptsLegacyGenerator(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	require.NoError(t, os.MkdirAll(filepath.Join(dir, ".harness"), 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".harness", "metadata.yaml"), []byte("schema_version: 1\ngenerator: mars-harness\ngenerator_version: 0.65.0\n"), 0o644))
...
func TestInit_success(t *testing.T) {
	...
	assert.Contains(t, manifestStr, "file_read, file_write, shell_exec, dependency_sync, grep", "QA should have bounded report-write, dependency hydration, and shell validation access")
	assert.Contains(t, manifestStr, "tool_inventory_audit, git_status, git_diff, git_commit, git_push]", "QA should have git inspection and report commit tools")
	assert.Contains(t, manifestStr, "mars_cli", "manifest should expose mars_cli as a mirrored tool")
	...
	assert.NotContains(t, ctoRoleBlock, "mars_cli", "fresh CTO bootstrap should not run source-harness CLI workflows before product tickets exist")
	...
	assert.Contains(t, string(ctoPrompt), "Create exactly one ordinary feature ticket")
	assert.Contains(t, string(ctoPrompt), "completed product ticket with build/smoke evidence exists yet")
	assert.Contains(t, string(ctoPrompt), "After the first product ticket has completed build/smoke evidence")
	assert.Contains(t, string(ctoPrompt), "treat that active-plan feature as the handoff gate")
	assert.Contains(t, string(ctoPrompt), "Ensure the first-slice ticket covers")
	assert.Contains(t, string(ctoPrompt), "not decompose the same BDD scenario")
	assert.Contains(t, string(ctoPrompt), "Affected files must be target-owned")
	assert.Contains(t, string(ctoPrompt), "Do not assume a Go CLI shape")
	...
	assert.Contains(t, string(ctoPrompt), "Never put `cmd/mars`")
	...
	metadata, err := ReadHarnessMetadata(dir)
	require.NoError(t, err)
	assert.Equal(t, "mars", metadata.Generator)
	...
	require.NoError(t, err)
	assert.Contains(t, string(githubAuthSkill), "mars auth github setup")
	assert.Contains(t, string(githubAuthSkill), "mars auth github clear-local")
	assert.Contains(t, string(githubAuthSkill), "github_auth_check")
	assert.Contains(t, string(githubAuthSkill), "no-redirect anonymous request")
	...
	assert.Contains(t, string(cliSkill), "mars_cli")
	...
	require.NoError(t, err)
	assert.Contains(t, string(changelog), "mars release notes")

	releaseDoc, err := os.ReadFile(filepath.Join(dir, "docs", "design-docs", "release-versioning.md"))
	require.NoError(t, err)
	assert.Contains(t, string(releaseDoc), "mars release backfill-notes")
	assert.Contains(t, string(releaseDoc), "--check")
	assert.Contains(t, string(releaseDoc), "Release Access")
	...
	assert.Contains(t, string(agentGuide), "mars mcp serve")
	assert.Contains(t, string(agentGuide), "mars_cli")
	...
	assert.Contains(t, string(agentGuide), "origin main")
	assert.Contains(t, string(agentGuide), "Product lifecycle improvements use a live evidence loop")
	assert.Contains(t, string(agentGuide), "classify every failure as foundation-owned, deployed-owned, or mixed/unclear")
	assert.Contains(t, string(agentGuide), "Do not turn MARS runtime")
	assert.Contains(t, string(agentGuide), "merge or fast-forward the confirmed fix to trunk")
	assert.Contains(t, string(agentGuide), "push it to the remote")
	assert.Contains(t, string(agentGuide), "claim improvement only from rerun evidence")
	...
	assert.Contains(t, string(agentGuide), "Operating rules inherited from MARS apply here")
	assert.Contains(t, string(agentGuide), "mars eject --repo .")
	assert.Contains(t, string(agentGuide), "--apply --confirm <repo-name>")
	assert.Contains(t, string(agentGuide), "repository's approved producer")
	assert.NotContains(t, string(agentGuide), "release publish-assets")
	assert.NotContains(t, string(agentGuide), "release verify-assets")
	assert.NotContains(t, string(agentGuide), "release audit")
	assert.Contains(t, string(agentGuide), "mars auth github check")
	assert.Contains(t, string(agentGuide), "mars auth github clear-local")
	assert.Contains(t, string(agentGuide), "checked anonymously first")
	...
	assert.Contains(t, string(harnessGlossary), "Live evidence improvement loop")
	assert.Contains(t, string(harnessGlossary), "Failure ownership classification")
	assert.Contains(t, string(harnessGlossary), "Foundation-owned failure")
	assert.Contains(t, string(harnessGlossary), "Deployed-owned failure")
	...
	assert.Contains(t, string(harnessGlossary), "mars_cli")
	...
	assert.Contains(t, string(harnessGlossary), "When classifying failures or planning live-loop fixes include this")
	...
	assert.Contains(t, string(toolsGlossary), "mars_cli")
	...
	assert.Contains(t, string(toolsGlossary), "mars mcp serve")
	...
	assert.Contains(t, string(toolsGlossary), "Browser-framework post-build note")
	assert.Contains(t, string(toolsGlossary), "generated-bundle inspection")
	...
	require.NoError(t, err)
	assert.Contains(t, string(qaPrompt), "Default QA is read-only except for bounded shell validation")
	assert.Contains(t, string(qaPrompt), "START with an allowed read-only tool call")
	assert.Contains(t, string(qaPrompt), "QA has shell_exec only for bounded validation evidence")
	assert.Contains(t, string(qaPrompt), "stale binaries from older")
	...
	assert.Contains(t, string(qaPrompt), "package/module initialization such as `go mod init`")
	assert.Contains(t, string(qaPrompt), "placeholder")
	assert.Contains(t, string(qaPrompt), "no `_test.go` files")
	assert.Contains(t, string(qaPrompt), "After the required build/test/runtime/docsync evidence has passed")
	assert.Contains(t, string(qaPrompt), "immediately record the approved or")
	assert.Contains(t, string(qaPrompt), "terminal-only boundary")

	orchestratorPrompt, err := os.ReadFile(filepath.Join(dir, ".harness", "roles", "orchestrator.md"))
	require.NoError(t, err)
	assert.Contains(t, string(orchestratorPrompt), "teaches conventions")
	assert.Contains(t, string(orchestratorPrompt), "never treat those examples as live")
	assert.Contains(t, string(orchestratorPrompt), "Never assume a")
	assert.Contains(t, string(orchestratorPrompt), "frontmatter such as `id: T-004`")
	...
	assert.NotContains(t, string(mirroredGlossary), "mars-linux-amd64")
	...
	assert.Contains(t, string(designIndex), "AD-252")
	assert.Contains(t, string(designIndex), "AD-266")
	assert.Contains(t, string(designIndex), "AD-267")
	assert.Contains(t, string(designIndex), "AD-268")
	assert.Contains(t, string(designIndex), "AD-269")
	...
	assert.Contains(t, string(cliToolSkillSync), "mars_cli")
	...
	assert.Contains(t, string(operatingModel), "Product lifecycle improvements use the same evidence loop as delivery work")
	assert.Contains(t, string(operatingModel), "fast-forward the confirmed fix to trunk")
	assert.Contains(t, string(operatingModel), "push it to the remote")
	assert.Contains(t, string(operatingModel), "improvement only from rerun evidence")
	assert.Contains(t, string(operatingModel), "Failure ownership classification is universal doctrine")
	assert.Contains(t, string(operatingModel), "source-only shorthand")
	...
	assert.Contains(t, string(cooPrompt), "Do not create tickets by another path")
	assert.Contains(t, string(cooPrompt), "file_write under")
	...
	assert.Contains(t, string(cooPrompt), "edit exactly that existing path")
	assert.Contains(t, string(cooPrompt), "docs/features/F-001-product-walking-skeleton.md")
	assert.Contains(t, string(cooPrompt), "Do not create a second feature contract path")
	assert.Contains(t, string(cooPrompt), "rewrite the existing")
	assert.Contains(t, string(cooPrompt), "advanced-only extensions")
	assert.Contains(t, string(cooPrompt), "Generic glue words")
	assert.Contains(t, string(cooPrompt), "must not imply that basic in-scope capabilities")
	...
	ctoPrompt, err = os.ReadFile(filepath.Join(dir, ".harness", "roles", "cto.md"))
	require.NoError(t, err)
	assert.Contains(t, string(ctoPrompt), "Do not create or edit product implementation files")
	...
	assert.Contains(t, string(engineerPrompt), "reference that exact path")
	assert.Contains(t, string(engineerPrompt), "never invent a second")
	...
	assert.Contains(t, string(engineerPrompt), "Static HTTP curl proves the files are served")
	assert.Contains(t, string(engineerPrompt), "node --check main.js")
	assert.Contains(t, string(engineerPrompt), "18081")
	assert.Contains(t, string(engineerPrompt), "18082")
	assert.Contains(t, string(engineerPrompt), "new Phaser.Game")
	assert.Contains(t, string(engineerPrompt), "npm run build")
	assert.Contains(t, string(engineerPrompt), "before that browser-product smoke passes")
	assert.Contains(t, string(engineerPrompt), "dist/assets")
	assert.Contains(t, string(engineerPrompt), "require('phaser')")
	assert.Contains(t, string(engineerPrompt), "defined or imported")
	assert.Contains(t, string(engineerPrompt), "this.add")
	...
	assert.Contains(t, string(engineerPrompt), "REVIEW REWORK FAST PATH")
	assert.Contains(t, string(engineerPrompt), "do not add new exploratory edge cases")
	assert.Contains(t, string(engineerPrompt), "rebuild it after every source change")
	assert.Contains(t, string(engineerPrompt), "CONTRACT-FIRST IMPLEMENTATION")
	assert.Contains(t, string(engineerPrompt), "The ticket acceptance criteria and feature contract are")
	assert.Contains(t, string(engineerPrompt), "the product contract for this run")
	assert.Contains(t, string(engineerPrompt), "before any packaging, install, distribution")
	assert.Contains(t, string(engineerPrompt), "repo-local build outputs unless the selected ticket explicitly requires")
	assert.Contains(t, string(engineerPrompt), "Extra edge cases discovered during implementation")
	assert.Contains(t, string(engineerPrompt), "Direct runtime commands that execute the ticket behavior")
	assert.Contains(t, string(engineerPrompt), "rather than waiting with empty shell commands")
	assert.Contains(t, string(engineerPrompt), "Treat the target project as its own product")
	assert.Contains(t, string(engineerPrompt), "Do not initialize")
	assert.Contains(t, string(engineerPrompt), "standard-library tests")

	qaPrompt, err = os.ReadFile(filepath.Join(dir, ".harness", "roles", "qa.md"))
	require.NoError(t, err)
	assert.Contains(t, string(qaPrompt), "MarsDocSync")
	assert.Contains(t, string(qaPrompt), "starting a static server is only")
	assert.Contains(t, string(qaPrompt), "Static curl proves")
	assert.Contains(t, string(qaPrompt), "missing build scripts are changes_requested")
	assert.Contains(t, string(qaPrompt), "Immediately after build passes")
	assert.Contains(t, string(qaPrompt), "Do not `require('phaser')`")
	assert.Contains(t, string(qaPrompt), "undefined scene callbacks")
	assert.Contains(t, string(qaPrompt), "unbound helper functions")

	securityPrompt, err := os.ReadFile(filepath.Join(dir, ".harness", "roles", "security.md"))
	require.NoError(t, err)
	assert.Contains(t, string(securityPrompt), "Do not run broad recursive secret scans through")
	assert.Contains(t, string(securityPrompt), "canonical browser-product smoke")
	assert.Contains(t, string(securityPrompt), "Do not patch product, test, ticket, or feature-contract files during Security")
	assert.Contains(t, string(securityPrompt), "record `changes_requested` for Engineer")
	assert.Contains(t, string(securityPrompt), "target-local case contract names an exact")
	assert.Contains(t, string(securityPrompt), "do not use the generic dated audit path")
	assert.Contains(t, string(securityPrompt), "Classify findings from current evidence, not speculation")
	assert.Contains(t, string(securityPrompt), "If the observed command already exits non-zero safely")
	assert.Contains(t, string(securityPrompt), "runtime may reject any")
	assert.Contains(t, string(securityPrompt), "**Required before release:** yes | no")
	...
	assert.Contains(t, string(dogfoodPrompt), "treat `src/` as the static server root")
	assert.Contains(t, string(dogfoodPrompt), "do not start a repo-root server first")
	...
	assert.Contains(t, string(dogfoodPrompt), "If an active ticket already covers the scenario")
	assert.Contains(t, string(dogfoodPrompt), "stop additional validation and do not create another")
	assert.Contains(t, string(dogfoodPrompt), "before any more shell validation or ticket_create")
	...
	assert.Contains(t, string(releaseDoc), "Agents should run these MARS workflows through `mars_cli`")
	assert.Contains(t, string(releaseDoc), "release: notes X.Y.Z")
	assert.Contains(t, string(releaseDoc), "repository owns its release producer")
	assert.NotContains(t, string(releaseDoc), "mars release publish-assets")
	assert.NotContains(t, string(releaseDoc), "mars release verify-assets")
	assert.NotContains(t, string(releaseDoc), "mars release audit")
	assert.Contains(t, string(releaseDoc), "vX.Y.Z")
	assert.Contains(t, string(releaseDoc), "artifact-contract gate")
	assert.Contains(t, string(releaseDoc), "publishes through GitHub Releases")
	assert.Contains(t, string(releaseDoc), "expected unique remote names")
	...
	assert.Contains(t, string(releasePrompt), "Run MARS CLI workflows through the `mars_cli` tool")
	...
	assert.Contains(t, string(releasePrompt), `["release","notes","--repo",".","--bump","auto","--dry-run"]`)
	...
	assert.Contains(t, string(releasePrompt), "repository's approved producer")
	assert.NotContains(t, string(releasePrompt), "release publish-assets")
	assert.NotContains(t, string(releasePrompt), "release verify-assets")
...
func TestInitWritesWorkspaceNoiseGitignore(t *testing.T) {
	...
	assert.Contains(t, string(content), ".vite/")
}
