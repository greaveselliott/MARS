...
func (r Report) Remediation() string {
	...
	return "update docs/roles/ROLES.md to match .harness/manifest.yaml, then run 'mars doctor --repo <path>'"
}
...
// DefaultMarkdown returns the generated registry for deployed starter harness roles.
func DefaultMarkdown() string {
	return registryMarkdown(false)
}

// SourceMarkdown returns the generated registry for this source harness.
func SourceMarkdown() string {
	return registryMarkdown(true)
}

func registryMarkdown(includeSourceOnly bool) string {
	...
	if includeSourceOnly {
		lines = append(lines, "- Source-only foundation roles are listed separately, are not generated into target manifests, and must not be treated as missing target roles.")
	}
	lines = append(lines,
	...
	if includeSourceOnly {
		...
		b.WriteString("\n")
		b.WriteString(roleTableMarkdown(SourceOnlyEntries()))
		b.WriteString("\n")
	...
	b.WriteString("\n")
	b.WriteString(roleTableMarkdown(DefaultEntries()))
	return b.String()
}

func roleTableMarkdown(entries []Entry) string {
	var b strings.Builder
...
// SourceOnlyEntries returns a copy of source-only role registry rows.
func SourceOnlyEntries() []Entry {
	out := make([]Entry, len(sourceOnlyEntries))
	copy(out, sourceOnlyEntries)
	return out
}

var sourceOnlyEntries = []Entry{
	{
		Role:               "foundation-maintainer",
		Origin:             "source-only",
		Domain:             "maintainer",
		Mode:               "foundation-build",
		TriggerSources:     "manual/operator invocation only",
		Schedule:           "none",
		Tools:              "file_read, file_write, shell_exec, dependency_sync, mars_cli, grep, code_index, code_search, code_snippet, code_trace, code_impact, workspace_hygiene, github_auth_check, record_decision, ticket_create, tool_create, persona_create, task_trace_summarize, docsync_audit, git_status, git_diff, git_commit, git_push, job_disposition_record, release_orchestrate, github_release_status, git_release_guard",
		TrustLevel:         "contributor",
		Guardrails:         "foundation/deployed classification, no validation-subject doctrine leakage, docsync, release publication, live-validation evidence, and git discipline",
		ModelRouting:       "reasoning",
		ScoringSignals:     "reusable source improvement quality, doctrine sync, release completeness, validation feedback generalization",
		EscalationBehavior: "classify findings as foundation-owned, deployed-owned, mirrored doctrine, or evidence-only before changing source or creating tickets",
	},
}
...
var defaultEntries = []Entry{
	{
		...
		Domain:             "planner",
		Mode:               "strategy",
		TriggerSources:     "schedule; returns disposition to orchestrator",
		Schedule:           "0 20 * * 0",
		...
		Guardrails:         "goal, scope, persona handoff, no mutating shell, trust, and git discipline",
		ModelRouting:       "reasoning",
		ScoringSignals:     "goal quality, decision clarity, downstream plan usefulness, feedback resolution",
		EscalationBehavior: "return disposition to orchestrator; route exec plans to COO and strategy advice to Head of Strategy when configured",
	},
	{
		...
		TriggerSources:     "dispatch/manual only for strategy_advice, executive_narrative, tradeoff_analysis, or goal_conflict",
		...
		TrustLevel:         "progressive planner write limited to strategy and goal-framing docs",
		Guardrails:         "strategy scope, CEO decision ownership, no ticket creation, no implementation, no mutating shell, and git discipline",
		ModelRouting:       "reasoning",
		ScoringSignals:     "strategy memo clarity, tradeoff quality, measurable bet quality, CEO decision usefulness",
		EscalationBehavior: "return disposition to orchestrator; route goal decisions back to CEO",
	},
	{
		Role:               "coo",
		Origin:             "default",
		...
		TriggerSources:     "orchestrator dispatch after goal decision or planning feedback",
		Schedule:           "chain-only",
	...
	},
	{
		...
		TriggerSources:     "schedule; orchestrator dispatch after COO planning or technical-ticket feedback",
		Schedule:           "0 21 * * 0",
		...
		Guardrails:         "architecture rationale, technical decomposition, BDD ticket evidence, no implementation/product-file writes, no mutating shell, trust, and git discipline",
		ModelRouting:       "reasoning",
	...
	},
	{
		...
		TriggerSources:     "schedule; orchestrator dispatch for ticket delivery, review, dogfood, idle work, intervention debt, and failed checks",
	...
	},
	{
		...
		TrustLevel:         "progressive reviewer write limited to QA reports",
		...
		EscalationBehavior: "return disposition to orchestrator; commit QA reports and record findings instead of hiding incomplete work",
	},
	{
		Role:               "security",
		Origin:             "default",
	...
	},
	{
		...
		TriggerSources:     "schedule; orchestrator dispatch after security or dependency signal",
		Schedule:           "0 23 * * 0",
		...
		Guardrails:         "dependency scope, tests, docsync, trust, release versioning, and git discipline",
		ModelRouting:       "fast",
	...
	},
	{
		...
		Guardrails:         "semantic versioning, changelog, release assets, docsync, trust, and git discipline",
		ModelRouting:       "reasoning",
	...
	},
	{
		...
		TriggerSources:     "schedule; orchestrator dispatch after implementation or failed dogfood signal",
		Schedule:           "0 10 * * 1-5",
	...
	},
	{
		...
		TriggerSources:     `orchestrator survey for Mars-recorded failed checks; optional GitHub workflow_run.conclusion == "failure" integration`,
		...
		Guardrails:         "pipeline scope, no recursive recovery, tests, docsync, trust, and git discipline",
		ModelRouting:       "coding",
		ScoringSignals:     "repair success, repeated failure suppression, check recovery time, regression rate",
		EscalationBehavior: "return disposition to orchestrator; record deterministic remediation when recovery repeats",
	},
	{
		Role:               "orchestrator",
		Origin:             "default",
		...
		TriggerSources:     "dispatch after every terminal job disposition",
		Schedule:           "dispatch-only",
		...
		EscalationBehavior: "choose a valid next manifest role or stop with a recorded reason",
	},
	{
		Role:               "janitor",
		Origin:             "default",
		Domain:             "orchestrator",
		Mode:               "ticket-hygiene",
		TriggerSources:     "schedule; ticket.stale_in_progress; orchestrator survey for stale, blocked, idle, and no-op ticket state",
		Schedule:           "0 7 * * *",
		...
		Guardrails:         "ticket lifecycle, active-plan hygiene, stale in-progress detection, docsync, trust, and git discipline",
		ModelRouting:       "fast",
...
func checkEntries(roles map[string]bundle.RoleConfig, entries map[string]Entry) Report {
	...
	for roleName, entry := range entries {
		...
		if isKnownSourceOnlyEntry(roleName, entry) {
			continue
		}
		report.add(RegistryPath,
...
func isKnownSourceOnlyEntry(roleName string, entry Entry) bool {
	if normalizeComparable(entry.Origin) != "source-only" {
		return false
	}
	for _, sourceEntry := range sourceOnlyEntries {
		if roleName == normalizeKey(sourceEntry.Role) {
			return true
		}
	}
	return false
