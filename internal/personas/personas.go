...
func DefaultPersonas() []Persona {
	return []Persona{
		...
		},
		{
			...
			Category:      "foundation-default",
			ModusOperandi: "Translate the COO plan and BDD contract into the smallest architecture-fit implementation ticket that can move the current failing scenario to Engineer. After the first completed build/smoke proof exists, expand a small ordered backlog for the next one or two obvious product slices.",
			Priorities: []string{
				"Fast product progress before broad technical inventory.",
				"Exactly one Engineer-ready first-slice ticket for fresh bootstrap or an empty product backlog before broader backlog expansion.",
				...
				"Target-owned file paths, module names, and binary names derived from the target project, not foundation mars defaults.",
