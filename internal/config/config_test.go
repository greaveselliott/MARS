...
func TestDefaultsDefaultPathAndSaveRoundTrip(t *testing.T) {
	...
	if got := DefaultPath(); got != filepath.Join(home, ".mars-harness", "config.yaml") {
		t.Fatalf("DefaultPath = %q", got)
	}

	cfg := Defaults()
	if cfg.ModelsDir != filepath.Join(home, ".mars-harness", "models") {
...
func TestLoadAppliesEnvironmentOverrides(t *testing.T) {
	t.Setenv("MARS_HARNESS_MODELS_DIR", "/tmp/models")
	t.Setenv("MARS_HARNESS_BIN_DIR", "/tmp/bin")
	t.Setenv("MARS_HARNESS_TRACES_DIR", "/tmp/traces")
	t.Setenv("MARS_HARNESS_LOG_FORMAT", "json")
	t.Setenv("MARS_HARNESS_GITHUB_TOKEN", "env-token")
	t.Setenv("MARS_HARNESS_PERFORMANCE_PROFILE", "battery")
	t.Setenv("MARS_HARNESS_LLAMA_FLASH_ATTENTION", "off")
	t.Setenv("MARS_HARNESS_TELEMETRY_REPORTING", "on")
	t.Setenv("MARS_HARNESS_TELEMETRY_ENDPOINT", "https://telemetry.example")
	t.Setenv("MARS_HARNESS_TELEMETRY_TOKEN", "telemetry-token")
	t.Setenv("MARS_HARNESS_TELEMETRY_INTERVAL", "1h")
	t.Setenv("MARS_HARNESS_WEBHOOK_PORT", "19091")
	t.Setenv("MARS_HARNESS_DASHBOARD_PORT", "19090")
	t.Setenv("MARS_HARNESS_LLAMA_PARALLEL", "2")
	t.Setenv("MARS_HARNESS_LLAMA_THREADS", "8")
	t.Setenv("MARS_HARNESS_LLAMA_THREADS_BATCH", "4")
	t.Setenv("MARS_HARNESS_LLAMA_BATCH_SIZE", "512")
	t.Setenv("MARS_HARNESS_LLAMA_UBATCH_SIZE", "128")
	t.Setenv("MARS_HARNESS_LLAMA_MLOCK", "yes")
