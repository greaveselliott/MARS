...
func checkTool(ctx context.Context, cfg Config) Component {
	...
	if err != nil {
		...
		component.Command = "mars-harness update tool"
		return component
	}
	...
	case selfupdate.VersionBehind:
		...
		component.Command = fmt.Sprintf("mars-harness update tool --version v%s", latest)
...
func checkHarness(cfg Config) Component {
	...
	if _, err := os.Stat(harnessDir); os.IsNotExist(err) {
		...
		component.Command = fmt.Sprintf("mars-harness init --repo %s", quotePath(absRepo))
		return component
	} else if err != nil {
	...
	case selfupdate.VersionAhead:
		component.Status = StatusAhead
		component.Message = "target harness was generated by a newer mars-harness"
	default:
		component.Status = StatusUnknown
		component.Command = fmt.Sprintf("mars-harness update harness --repo %s", quotePath(absRepo))
