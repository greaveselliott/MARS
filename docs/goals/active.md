...
- Current Evidence: The private history is reconstructed from `v0.68.49`; 69 audit-era tags and Releases are retired; the repository remains private; T-065 passed the pinned GoReleaser producer contract and retired the bespoke publisher; T-067 passed the exact Go 1.25.12 source compatibility prerequisite; and T-066/F-018-S002 passed through `7fe152c`. Its A1/A2/B1/B2/B3 checkpoints authenticate signed checksums and canonical archives, acquire one immutable verified candidate, durably replace or restore the binary, and wire release-mode updates before PATH repair; C/D retire circular shell bootstrap and weaker parallel consumers; E passes the focused normal/race and exact vulnerability/fuzz/lint/DocSync/cross-build tail plus an isolated-prefix source candidate, clean fresh target, and preverified-candidate update/rollback lifecycle without claiming a real MARS signature or release. T-068 checkpoint A passed exact-head private GitHub Actions run `29894376197` at `aa4a16b` with two clean snapshot roots, native Linux/source fixtures, offline consumer rollback, zero uploaded artifacts, successful cleanup, private visibility, and no `v0.69.0` tag or Release. Checkpoint B passed at the same immutable commit on macOS `26.3.1` (Darwin `25.3.0`)/arm64 with exact Go `1.26.5`, supported source and unsigned-native fixtures, equivalent targets, authority-free dry-run, and the exact five offline consumer/install/update/rollback tests; independent cleanup verification proved no rehearsal-root residue after a truthfully recorded cleanup-status false negative. T-069 then passed exact-head source-compatibility run `29898672813` at `03008f7` in both supported Go lanes plus the expected below-minimum rejection lane.
- Current Blocker: T-068 checkpoint C final immutable-state reconciliation and sign-offs remain incomplete. Fresh packaged bootstrap still lacks a non-circular trust mechanism; complete notices and the recorded producer findings remain cutover no-go items; and the standard history/privacy/IP, runtime, contribution, cutover, and canary gates remain incomplete. Primary Status remains `primary_blocked`; no supported public release exists.
