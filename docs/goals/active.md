...
- Current Evidence: The private history is reconstructed from `v0.68.49`; 69 audit-era tags and Releases are retired; the repository remains private; T-065 passed the pinned GoReleaser producer contract and retired the bespoke publisher; T-067 passed the exact Go 1.25.12 source compatibility prerequisite; and T-066/F-018-S002 passed through `7fe152c`. Its A1/A2/B1/B2/B3 checkpoints authenticate signed checksums and canonical archives, acquire one immutable verified candidate, durably replace or restore the binary, and wire release-mode updates before PATH repair; C/D retire circular shell bootstrap and weaker parallel consumers; E passes the focused normal/race and exact vulnerability/fuzz/lint/DocSync/cross-build tail plus an isolated-prefix source candidate, clean fresh target, and preverified-candidate update/rollback lifecycle without claiming a real MARS signature or release. T-068/F-018-S003 passed its exact private Ubuntu and macOS source/unsigned-snapshot fixtures, offline consumer rollback, release-note dry-run, exact cleanup, immutable private-state reconciliation, and all persona sign-offs. T-069 restored the supported Linux source gate. On 2026-08-08, T-070 froze 302 canonical publication refs at `d04f642`, materialized all 12,002 reachable objects, and completed exact Gitleaks/TruffleHog Git-history and raw-object lanes with zero accepted-scan errors or skips. The Gitleaks results reconciled to owner-confirmed synthetic group `a2a292e31d652f22`; an isolated local-MARS review classified URI group `e32927624f4a2cac` as synthetic test stubs. A newly detected broad group contains 36 occurrences reducing mechanically to four distinct values and remains unresolved under opaque ID `f3dc0e336620abc6`.
- Current Blocker: T-070 group `f3dc0e336620abc6` is `real_or_unknown_rotation_required`: bounded local-MARS reviews could not establish that any of its four distinct values is synthetic. The owner must rotate or revoke all four outside the agent boundary before a newly frozen four-lane rescan. GitHub-hosted content review, manual privacy/IP/provenance/name review, and owner disposition also remain incomplete. Fresh packaged bootstrap still lacks a non-circular trust mechanism; complete notices and the recorded producer findings remain cutover no-go items; and runtime, contribution, cutover, and canary gates remain incomplete. Primary Status remains `primary_blocked`; no supported public release exists.
...
- Review Trigger: Any T-066 through T-068 evidence, standard scan finding, owner disposition, F-017 scenario change, private rehearsal, visibility change, cutover smoke, or canary incident.
