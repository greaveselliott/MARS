...
- Current Evidence: The private history is reconstructed from `v0.68.49`; 69 audit-era tags and Releases are retired; the repository remains private; T-065 passed the pinned GoReleaser producer contract and retired the bespoke publisher; T-067 passed the exact Go 1.25.12 source compatibility prerequisite; T-066 A1 at `fcf7397` authenticates the offline signed-checksum contract; and A2 at `b824b91` authenticates and safely inspects the canonical platform archive and release build metadata without filesystem mutation or execution.
- Current Blocker: T-066 checkpoints B through E, T-068, and the standard history/privacy/IP, runtime, contribution, rehearsal, cutover, and canary gates remain incomplete. No supported public release exists.
