...
- Current Ticket: T-078, blocked on separately re-approving the revised sanitation transaction after deployment-state drift. Exact source `d411cbe` supplies the dormant least-privilege Go/archive/Syft/GitHub-attestation workflow and compatible consumer and passes the two-clean-root no-publish rehearsal. Exact pushed source `a4bbf81` and hosted run `32848968969` pass the owner-approved Go 1.25.13 source-floor remediation without a vulnerability exception. No launch tag, real attestation, upload, Release, visibility change, announcement, or publication is authorized.
