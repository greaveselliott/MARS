...
- Current Blocker: Fresh packaged bootstrap still lacks a non-circular trust mechanism; T-068 private two-build and clean macOS/Linux rehearsal is not yet created or run; complete notices and the recorded producer findings remain cutover no-go items; and the standard history/privacy/IP, runtime, contribution, cutover, and canary gates remain incomplete. Primary Status remains `primary_blocked`; no supported public release exists.
