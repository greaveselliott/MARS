...
- Current Ticket: T-078, blocked on the separately approved mutation and hosted-CI closure of the standard path. Exact source `d411cbe` supplies the dormant least-privilege Go/archive/Syft/GitHub-attestation workflow and compatible consumer and passes the two-clean-root no-publish rehearsal. The current hosted seal binds the exact cleanup sets, but no hosted deletion or immutable-Release setting change is yet authorized. No launch tag, real attestation, upload, Release, visibility change, announcement, or publication is authorized.
- Current Checkpoint Evidence: T-077 is complete through exact pushed commits `10b62f7d59620022b2e1030c5f33856d0c16e70f`, `04d6ba6844126dc84eb6bedc13c78bd31f8d371d`, `85c689c70ef801a2747acabf537739c9ebad3c12`, and closure source `56b8de336cf4d1439944cc7eb8ea0f5ad4043f2b`. T-078's rejected GoReleaser/Cosign/containment route and verifier evidence are preserved as a local non-authorizing checkpoint. Exact pushed source `d411cbe` replaces that route with the dormant AD-315 workflow, standard attestation consumer, Go 1.27 producer contract, zero-called vulnerability baseline, and a passed two-clean-root no-publish rehearsal. Current read-only hosted evidence binds 301 tags, the intended 56 Releases, 500 assets, 473 completed runs, 77 deployments, zero Actions artifacts/caches, and disabled immutable Releases. The oldest 401 and next 65 run partitions reproduce their prior digests; the seven-run delta is acquired and scan-clean with zero authoritative scanner errors or unresolved candidates. The latest hosted run remains red because Go 1.25.12 now has six called standard-library vulnerabilities fixed in 1.25.13; no exception is broadened. Evidence is in `docs/validation/reports/2026-08-24-t078-hosted-state-revalidation.md`.
- Current Blocker: Separate approval and postconditions for the exact cleanup/future-only immutable-Release transaction, a green accepted-source hosted run without broadening the vulnerability disposition, contribution controls, separately approved visibility change, two real attested releases, and canary remain. Primary Status is `primary_blocked`.
