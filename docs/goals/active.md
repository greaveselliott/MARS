...
- Current Ticket: T-076 is current after bounded CTO-weekly, QA, and Security planning at clean synchronized `c04a172`. Checkpoint A alone is authorized first: default all execution entry points to an observer ceiling, require explicit host acknowledgement before target mutation or model-controlled host execution, and fail unsupported isolation before mutation. Observer may retain owner-only operational bookkeeping. Sanitized-environment, job-owned-process, owner-only-state, redaction, and trace-lifecycle checkpoints remain planned rather than implemented.
