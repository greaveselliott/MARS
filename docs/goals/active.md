...
- Current Ticket: T-058 is current after T-076 closed under the owner governor. Checkpoint A is complete at exact pushed commit `9191182601d79b996f1848a1e867e50b7d6eaf1c`: all five execution entry points default to an observer ceiling, acknowledged host does not upgrade configured role trust, and unsupported isolation fails before effects. Checkpoint B1 is complete at exact pushed commit `5c23f536fadd9ab18694e3f46ed9b10ca96594da`: one thin child-environment seam gives every named shell, MARS CLI, dependency, Git, code-intelligence, managed-inference, MCP stdio, Jira, GitHub-auth, and source-update child an explicit environment; ordinary non-sensitive runtime/toolchain state remains, credential-like names are filtered, parent-only `MARS_CHILD_ENV_ALLOWLIST` never propagates, and Jira requires the repository request plus owner allowlist. This is name-based exposure reduction, not containment. Exact B1 gates passed: `go test ./internal/childenv ./internal/tools ./internal/inference ./internal/serve ./internal/codeintel ./internal/githubauth ./internal/selfupdate ./internal/mcpclient ./internal/jira ./cmd/mars -count=1`; `GOCACHE=/private/tmp/mars-t076b1-root-race-cache go test -race ./internal/childenv ./internal/tools ./internal/inference ./internal/jira -run 'Test(Filter|ApplyWith|ShellExecSanitizes|DependencySyncChild|ServerManagedChild|ProxyEnvironment)' -count=1`; `GOCACHE=/private/tmp/mars-t076b1-root-cache go vet ./internal/childenv ./internal/tools ./internal/inference ./internal/serve ./internal/codeintel ./internal/githubauth ./internal/selfupdate ./internal/mcpclient ./internal/jira ./cmd/mars`; `go test ./internal/docsconsistency ./internal/docsync -count=1`; `go run ./cmd/mars docsync audit --repo .` with 364 files and zero findings; and `git diff --check`. Real-child regressions cover foreground/background shell, dependency PATH/cache preservation, managed inference, and the Jira dual gate. QA, Security, Release Manager, and Orchestrator returned GO. Checkpoint B2 is complete at exact pushed commit `473b829efe865630f4942b55af9e0108d7529d0c`: process records require a job ID; listing, direct helper policy, kill, and bounded TERM-then-KILL cleanup are job-scoped; unowned and cross-job targets fail closed; every named lifecycle hook cleans only its owned groups; and unrelated listeners and llama-server-named processes survive. Exact B2 gates passed: `go test ./internal/tools ./internal/mcpstdio ./internal/serve ./internal/scanner ./cmd/mars`; `go test -race ./internal/tools ./internal/mcpstdio ./internal/serve ./cmd/mars`; `go vet ./internal/tools ./internal/mcpstdio ./internal/serve ./internal/scanner ./cmd/mars`; `go test ./internal/docsync ./internal/docsconsistency`; `go run ./cmd/mars docsync audit --repo .` with 364 files and zero findings; and `git diff --check`. QA, Security, and Orchestrator returned GO. The persisted-config credential correction at 9eb3f96 and public shared-database path correction at 31b00b1 complete T-076's narrowed launch scope; broader local-state hardening is deferred. T-058 and F-017-S002 remain incomplete. The repository stays private at `VERSION=0.68.49` and `primary_blocked`; legal/rights and installed-App no-gos remain, and no Release, settings, visibility, signing, publication, or announcement authority changed.
- Current Blocker: On 2026-08-09 the owner directed that `MARS` be retained despite the materially overlapping official-register finding. T-073 therefore requires qualified trademark counsel's written disposition for the intended use and launch territories and explicit owner authority over retained first-party, predecessor, Cursor/automation, and historical PNG material before final `preserve_audited_history` attestation can pass. T-073 is parked on that owner-only/legal hold and must resume and pass before T-080; T-077 through T-079 may proceed privately and sequentially. Primary completion remains blocked on T-073, resumed T-058, and T-077 through T-081: rights/name/history disposition; reachable runtime hardening; anonymous signed distribution; fork-safe controls including the installed-App finding; final releases; public cutover; and a clean 48-hour canary. Primary Status is `primary_blocked`.
