...
- Current Evidence: The private history is reconstructed from `v0.68.49`; 69 audit-era tags and Releases are retired; the repository remains private; T-065 passed the pinned GoReleaser producer contract and retired the bespoke publisher; and T-066 now owns the fail-closed signed archive consumers with `v0.69.0` reserved for the future cutover.
- Current Blocker: T-066 A1 rejected the Go-1.22-compatible Sigstore verifier, then found zero called vulnerabilities on the exact `sigstore-go v1.2.2` path when built with Go 1.25.12. The owner approved that source-floor change on 2026-07-22; T-067 must now establish the compatibility prerequisite before T-066 resumes, while T-068 and the standard history/privacy/IP, runtime, contribution, rehearsal, cutover, and canary gates also remain incomplete. No supported public release exists.
