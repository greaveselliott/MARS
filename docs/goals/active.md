...
- Current Ticket: T-078, blocked at producer admission before execution. The ticket must replan its production Go patch and exact GoReleaser dependency graph, repeat structured called-symbol admission, and only then resume the split signer and exact-ten uploaded-asset rehearsal. Read-only hosted acquisition may continue, but no launch tag, signature, upload, Release, visibility change, announcement, hosted deletion, or immutable-Release setting change is authorized.
- Current Checkpoint Evidence: T-077 is complete through exact pushed commits `10b62f7d59620022b2e1030c5f33856d0c16e70f`, `04d6ba6844126dc84eb6bedc13c78bd31f8d371d`, `85c689c70ef801a2747acabf537739c9ebad3c12`, and closure source `56b8de336cf4d1439944cc7eb8ea0f5ad4043f2b`. T-078 then authenticated exact GoReleaser v2.17.1 through SumDB, built it with CGO disabled and Go 1.26.5, and stopped before execution when exact govulncheck v1.6.0 found 12 called vulnerability IDs and 104 terminal called symbols. Separately, the oldest 401 live workflow-run IDs reproduced T-072's seal; the exact 65-run delta at 466 completed and zero active runs was acquired and returned zero findings, errors, skips, timeouts, rejected inputs, or unresolved candidates from both admitted offline scanners. The durable record is `docs/validation/reports/2026-08-24-t078-release-production-admission-blocked.md`. Every later run remains in scope, and hosted jobs still fail before step one on GitHub Billing & plans.
- Current Blocker: T-078's selected producer is not admissible: exact GoReleaser v2.17.1 built with the stale Go 1.26.5 release toolchain has 12 called vulnerabilities, including two without a fixed dependency version. Replan to a current supported Go patch and exact scan-clean producer graph or obtain a separately authorized qualified disposition; no such disposition currently exists. Independently, T-073 still requires qualified trademark counsel's written disposition for retaining `MARS` and the owner's signed authority/history attestation. The two all-repository write-capable Apps and GitHub Billing & plans condition remain launch no-gos. T-079 cannot advance under the sequential plan until T-078 passes. Primary Status is `primary_blocked`.
