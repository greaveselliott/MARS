...
- Current Evidence: The private history is reconstructed from `v0.68.49`; 69 audit-era tags and Releases are retired; the repository remains private; T-065 passed the pinned GoReleaser producer contract and retired the bespoke publisher; T-067 passed the exact Go 1.25.12 source compatibility prerequisite; T-066 A1 at `fcf7397` authenticates the offline signed-checksum contract; A2 at `b824b91` safely inspects the canonical platform archive and release build metadata; B1 at `f3ed495` acquires a replay- and drift-checked verified candidate without filesystem mutation or execution; B2 at `92d7ddd` provides a network-free durable replacement transaction with verified compensation or explicit recovery-required evidence; B3 at `683daf8` routes release-mode updates through B1 and B2 with exact running-binary drift binding before PATH repair; C at `f45d5d2` retires the circularly untrusted shell binary bootstrap to a non-mutating reviewed-source route; and D through `64c7d3b` retires weaker verifier/audit consumers and synchronizes CLI, generated, BDD, design, product, and operator guidance.
- Current Blocker: T-066 checkpoint E, unavailable fresh packaged bootstrap pending a non-circular trust mechanism, T-068, and the standard history/privacy/IP, runtime, contribution, rehearsal, cutover, and canary gates remain incomplete. Primary Status remains `primary_blocked`; no supported public release exists.
