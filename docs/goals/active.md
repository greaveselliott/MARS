...
- Current Ticket: T-077, created through `ticket_create` as the bounded F-017-S003 enabler for anonymous setup/auth, local fallback clearing, explicit third-party download acknowledgement, and exact-version Go/SumDB bootstrap into the existing signed updater. Checkpoint A passed at exact pushed commit `10b62f7d59620022b2e1030c5f33856d0c16e70f`; Checkpoint B passed at exact pushed commit `04d6ba6844126dc84eb6bedc13c78bd31f8d371d`; Checkpoint C implementation passed at exact pushed commit `85c689c0a73215ab887542e012aac66ca1b43c40`. T-077 remains incomplete pending the pre-existing T-075 `internal/agent` Git-fixture correction, four CGO-disabled builds, and clean credential-free installed macOS/Linux source/setup evidence. Real tags, Releases, signing, logged-out public lifecycle proof, and unsupported automatic Linux llama.cpp acquisition remain outside T-077. The repository stays private at `VERSION=0.68.49` and `primary_blocked`; legal/rights and installed-App findings remain no-gos, and no Release, settings, visibility, signing, publication, or announcement authority changed.
- Current Checkpoint Evidence: T-077 Checkpoint A is a bounded 33-path packet. Ordinary setup performs no auth step; the exact no-redirect official metadata probe is anonymous first; only an exact `401`, `403`, or `404` may resolve optional credentials for one same-origin/path retry; `auth github clear-local` removes only the stored config fallback; and doctor retains its selected custom `ConfigPath`. Affected normal/race tests, vet, docs consistency, DocSync across 364 files with zero findings, formatting, and diff gates passed with QA, Security, and Orchestrator GO. Checkpoint B is a bounded 21-file packet: setup stable-sorts one unique pending-artifact plan with the concrete bundle, immutable identities, exact sizes, license IDs/URLs, and terms/notice URLs; interactive setup confirms once, while non-TTY/JSON requires exact `--download --yes` and emits the complete JSON plan before requests. Exact plan and downloader identity checks make decline, missing acknowledgement, incomplete provenance, or changed state zero-request and zero-download-artifact-write outcomes. Skip, test, deferred, and cloud paths need no acknowledgement; no legal attestation is persisted; automatic Linux llama.cpp acquisition remains disabled. Exact affected normal/race tests, affected vet, docs consistency, DocSync across 366 files with zero findings, formatting, and diff gates passed with QA, Security, and Orchestrator GO. Checkpoint C is a bounded 13-file packet: direct privileged Bash startup suppresses inherited functions and `BASH_ENV`; optional GitHub tokens stay out of `env(1)` arguments and Go and are scoped only to the staged updater after their descriptors close; exact Go uses the public proxy/SumDB, trusted private staging, and `-modcacherw`; runtime `BuildInfo` proves canonical command/module, exact tag, canonical SHA-256 `h1`, and no replacements before signed acquisition/replacement; and failure/cleanup results remain truthful. Normal/race tests, vet, hostile Bash 3.2 installer and descriptor proof, CLI/help mirrors, docs consistency, DocSync 366/0, and diff gates passed with QA, Security, Release Manager, and Orchestrator GO. The existing signed release consumer remains the sole archive/signature verifier.
- Current Blocker: On 2026-08-09 the owner directed that `MARS` be retained despite the materially overlapping official-register finding. T-073 therefore requires qualified trademark counsel's written disposition for the intended use and launch territories and explicit owner authority over retained first-party, predecessor, Cursor/automation, and historical PNG material before final `preserve_audited_history` attestation can pass. T-073 is parked on that owner-only/legal hold and must resume and pass before T-080; T-077 through T-079 may proceed privately and sequentially. Primary completion remains blocked on T-073 and T-077 through T-081: rights/name/history disposition; anonymous signed distribution; fork-safe controls including the installed-App finding; final releases; public cutover; and a clean 48-hour canary. Primary Status is `primary_blocked`.
