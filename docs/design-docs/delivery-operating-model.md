...
| GitHub Releases page stays stale after tags | An authorized publication workflow did not complete or the release object is incomplete | Keep publication blocked, preserve immutable refs, and follow the repository-approved recovery workflow. Never create a notes-only fallback or infer completion from transport success when the artifact contract is unresolved. |
