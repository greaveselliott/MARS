...
## AD-222: Ticket Shaping Advances Scenarios Instead Of Looping
...
The next live `demo-6` observation exposed a follow-on lifecycle stall. The
target had one completed ordinary product ticket for `F-002-S001`, while the
active plan still named `F-002-S001` as the current failing scenario and listed
later scenarios in the schedule. CTO repeatedly tried to hand off
implementation, the Engineer ticket gate correctly refused to enqueue Engineer
without an open ordinary product ticket, and dispatch rewrote the route back to
`cto-weekly` for ticket shaping. Because the loop guard ran before that
post-gate rewrite, the same `cto-weekly -> cto-weekly` route repeated against
the same ticket-state hash instead of stopping or producing the next product
ticket.

The same run also showed CTO calling `ticket_create` with an empty
`bdd_scenarios` array. Policy blocked the mutation, but the repair guidance did
not tell CTO to use the current or next uncovered Scenario Schedule entry, so
the role had no clear product-progress path after the first ticket was done.

### Decision

Dispatch now applies a second loop guard after the Engineer ticket prerequisite
has rewritten an implementation handoff to CTO ticket shaping. If the same
source role, ticket ID, next need, next role, and routing ticket hash have
already repeated twice, dispatch records an operator-visible loop stop and does
not enqueue another CTO, Engineer, or Orchestrator bounce.

Generated CTO guidance now treats the active plan's Scenario Schedule as the
ticketing lane. CTO starts with the current failing scenario, but if that
scenario already has an ordinary product ticket in backlog, in-progress,
in-review, or done, CTO advances to the next uncovered scheduled scenario and
passes that scenario ID in a non-empty `bdd_scenarios` array. If all scheduled
scenarios already have tickets and no open product ticket remains, CTO routes
feedback to COO to update or close the plan instead of confirming stale work.
The `ticket_create` policy error for empty feature scenarios now names the
non-empty array repair shape and points at the next uncovered scenario.

### Consequences

- A stale active-plan pointer can no longer create an unbounded CTO shaping
  loop after the first product ticket completes.
- The factory still creates at most one ordinary product ticket per selected
  scenario, but it can continue to the next product slice once the previous
  slice is ticketed and no open product work remains.
- Empty `bdd_scenarios` ticket attempts should be repaired toward product BDD
  scenario IDs rather than becoming another silent no-progress loop.
