...
No-op loop note: repeated Engineer no-op `shell_exec` calls are phase-aware.
...
disposition. After validation and dirty ticket or product work, repeated
no-ops route to evidence update, commit, lifecycle completion, and QA handoff.
...
bypass. Helper scripts, arbitrary chained shell control syntax, runtime probes,
cleanup, ticket moves, and root scratch validation files stay blocked while
the lane is unresolved.
The only cleanup exception is non-recursive `rm` or `unlink` of test-like
files written by the same Engineer job after the test/build failure began;
unmarked tests, product source files, recursive removal, and ordinary cleanup
remain blocked.
