...
- Execution profile is an admission boundary independent of role trust. The
  default `observer` profile blocks every classified mutating tool, including
  `shell_exec` and `git_branch`, even if requested or stored trust is
  contributor or autonomous. `host` requires
...
The direct repository file tools (`file_read`, `file_write`, `file_search`, and
`grep`) resolve entries through a descriptor-contained standard-library root
and reject observed symlink parents and leaves. `file_write` uses an exclusive
...
| `file_search` | Find files by glob-style path patterns. | Non-mutating. Use for inventory before broad reads. Inventory stays descriptor-contained and rejects observed symlink traversal. |
| `grep` | Search file contents with a regex. | Non-mutating. Use to locate symbols, text, or repeated patterns. Matching files are opened relative to the descriptor-bound repository root; symlinked paths and failed admissions fail closed. |
...
| `release_orchestrate` | Plan and preflight a repository release workflow without exceeding its active version, tag, production, or publication authority. MARS source follows F-018 and is snapshot-only through T-068; targets use their own approved producer. | Planning workflow. It reports the allowed sequence; actual changes still use `mars_cli` and git tools under their own gates. |
...
- Need to verify public or optional authenticated MARS release access before
  update, release verification, install repair, or version-drift remediation: use
...
- Need to discover or invoke the universal tool surface from an operator shell
  ...
  `--trust contributor --execution-profile host
  --acknowledge-host-execution` only for deliberate mutating tool calls.
- Need an MCP-compatible client or local harness agent to see MARS tools
  ...
  contributor session additionally requires `--execution-profile host
...
- Need a dispatch-mode handoff, blocker, review request, no-work outcome, or
  ...
  be auto-committed only by acknowledged-host server execution when it is the
  only dirty path, but any product, ticket, documentation, or source change
...
- Need to inspect generated dependency/build churn before a job, commit, or
  ...
  be auto-repaired by acknowledged-host `serve` as a `.gitignore`-only commit
  when generated paths are untracked and `.gitignore` has no user changes.
  Observer `serve` reports but does not repair it. Host OS metadata such as
...
- Need a command outside the built-in tool surface: first require acknowledged
  host execution, then use `shell_exec`, keep the command narrow, and record
  any reusable gap as a tool improvement.
