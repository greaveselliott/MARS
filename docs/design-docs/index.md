...
| [orchestrated-organization-layer.md](orchestrated-organization-layer.md) | Accepted | Opt-in dispatch-mode organization/liveness layer around the queue and manifest roles, with recorded job dispositions and deterministic next-role routing. AD-090. |
...
| AD-090 | Dispatch-mode orchestration adds an opt-in organization/liveness layer around the existing queue and role pipeline, using deterministic routing, recorded job dispositions, and repo-owned ticket truth while legacy chaining remains the default. | orchestrated-organization-layer.md | Orchestration |
