...
| [orchestrated-organization-layer.md](orchestrated-organization-layer.md) | Accepted | Default dispatch-mode organization/liveness layer around the queue and manifest roles, with recorded job dispositions returning to Orchestrator for next-role selection. AD-090. |
| [dogfood-and-decisions.md](dogfood-and-decisions.md) | Accepted | Containerised E2E validation (Podman + native fallback), decision recording tool, strict-trunk pipeline for local use, ticket-drain discipline, recovery loop containment, queue self-healing, tool-creation scaffolding, mirrored CLI interaction, MCP tool exposure, bootstrap trust defaults, destructive shell gates, intervention-debt signal dedupe, and file-count blast-radius calibration. AD-021 through AD-030, AD-033, AD-060, AD-062, AD-077, AD-079, AD-087, AD-091 through AD-094. |
...
| AD-089 | Native Orchestrator surveys are an internal signal source that routes unattended ticket, check, dogfood, no-op, telemetry, score, recovery, and stuck-work state into bounded queue jobs or intervention-debt tickets with payload mode, concurrency group, and daily cap metadata. | trigger-orchestration.md | Orchestrator |
| AD-090 | Dispatch-mode orchestration is the generated default: terminal job dispositions return to Orchestrator for next-role selection, while legacy chaining remains supported for existing manifests. | orchestrated-organization-layer.md | Orchestration |
...
| AD-094 | File count is disabled as a default blast-radius gate, stronger line/deletion/secret/trunk checks remain enforced, and only high-priority intervention debt preempts ordinary backlog work. | dogfood-and-decisions.md | Safety |
