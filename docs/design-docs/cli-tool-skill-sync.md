...
The `mars` CLI is the primary control surface for operators, local
...
`mars_cli` tool, role allowlists, generated target guidance, and compact
skills that describe recurring workflows.

That creates a stale-documentation risk with real product impact. A command can
be added to `cmd/mars/main.go`, but agents may still operate from an old
`mars_cli` reference. A command can gain or change a `--repo` flag, but
...
MARS treats CLI-to-tool-and-skill synchronization as a foundation
operating rule.

Whenever the `mars` CLI changes, the same change must review and update
the mirrored CLI surfaces:

- `mars_cli` reference text;
- `mars_cli` repo shortcut support map;
...
The source of truth remains the Cobra command tree in `cmd/mars`. The
...
```mermaid
flowchart TD
    A["cmd/mars Cobra command tree"] --> B["Public CLI commands and flags"]
    B --> C["mars_cli reference"]
    B --> D["mars_cli repo shortcut map"]
...
`cmd/mars/main.go` owns the actual command tree, command aliases,
...
`internal/tools/mars_cli.go` exposes two important mirrors:
...
`MARS_CLI_BIN` first, then the active running harness executable, then
...
- `go test ./cmd/mars -run TestMarsCLI` compares the live Cobra
  command tree with the `mars_cli` reference and repo shortcut map.
- `go test ./internal/tools -run TestMarsCLI` verifies the tool behavior
  directly.
...
| --- | --- |
| `mars_cli` reference | Command path, flags, examples, mutability, dry-run guidance, JSON guidance. |
| `mars_cli` repo shortcut map | Whether `repo:"."` should append `--repo <abs path>` or be rejected. |
| Tool glossary | Whether agents should prefer the CLI tool, a specific registered tool, or a skill. |
| Generated role allowlists | Whether target agents need `mars_cli` or a narrower tool for the workflow. |
...
Update `internal/tools/mars_cli.go` whenever:
...
- Every runnable Cobra command is documented in the `mars_cli`
  reference.
- Every runnable command with a local workspace `--repo` flag is accepted by
  the `mars_cli` repo shortcut map.
...
- Tool-selection guidance explains when to use `mars_cli` versus a
...
| --- | --- |
| New command exists but agents cannot discover it | Command-tree test fails until `mars_cli` reference is updated. |
...
- CLI command tree changes cannot pass `go test ./cmd/mars` unless the
  `mars_cli` reference covers every runnable command.
- Commands with local workspace `--repo` flags cannot drift from the
  `mars_cli` repo shortcut map.
