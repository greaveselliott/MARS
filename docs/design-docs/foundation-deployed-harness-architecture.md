...
| --- | --- | --- | --- |
| Publishing `mars-harness` binary assets | Yes. Source release workflow tags `vX.Y.Z`, GitHub Release objects, binary assets, checksums, and `release verify-assets` are source release mechanics. | No. | No, unless a target separately chooses an equivalent binary release model. |
