...
| --- | --- | --- |
| Install and setup | installed `mars` command, `mars setup`, `mars auth github check`, `mars path setup`, config, model and binary cache | Run the harness from any directory without requiring GitHub credentials, classify official release access anonymously first, configure optional private-fork auth and provider credential names without exposing secret values, configure the user's shell PATH automatically, detect hardware, install local inference, choose a sensible performance profile, and explain missing prerequisites. |
...
`mars setup`, anonymous-first release access, optional private-fork auth, inference mode, local bundles, machine
...
| `mars auth github check` / `setup` / `clear-local` | Implemented | `check` makes one exact, no-redirect anonymous request to the official `api.github.com` release-metadata endpoint. Only an exact `401`, `403`, or `404` may resolve optional credentials and retry the same origin and path once; output classifies access as `anonymous`, `authenticated`, or `unavailable` without disclosure. `setup` persists optional private-fork or rate-limit auth. `clear-local` removes only the stored config `github_token`, preserving environment, GitHub CLI and GitHub App credentials, repositories, and remote state. |
| `mars setup` | Implemented, still hardening | Creates `~/.mars/` without requiring GitHub credentials, configures supported shell profiles for the installed command, writes config, detects hardware, installs llama.cpp server artifacts, downloads pinned models, and keeps optional integration setup explicit. |
...
| `./scripts/install.sh vX.Y.Z <absolute-install-dir>` | Implemented; official lifecycle blocked pending cutover | Runs directly from an independently reviewed checkout at the same exact stable tag; a network-fetched script-to-shell route and explicit shell-interpreter invocation are unsupported. Its privileged Bash shebang suppresses inherited functions and `BASH_ENV`, then starts the real body with only `PATH`, `HOME`, and `TMPDIR`; optional GitHub tokens cross over dedicated descriptors, stay absent from Go, and enter only the staged signed updater. Requires Go 1.25.13+ for first bootstrap, resolves one absolute Go executable, disables Go auth and CGO, neutralizes inherited build controls, applies `-modcacherw`, validates every resolved temporary-root ancestor, and builds only the canonical MARS module through the public Go proxy and SumDB in owner-only staging. The staged command independently validates its running canonical command/module, exact tag, canonical SHA-256 `h1` sum, and no replacements before the signed updater receives the same tag/final directory with shell PATH mutation skipped. Pre-commit rejection leaves an existing binary unchanged; recovery-required results preserve transaction evidence and require trusted-source repair. Successful script exit requires verified staging removal; incomplete cleanup preserves the original failure plus a path-free warning or, after installation, reports installed-with-incomplete-cleanup. No supported launch tag exists yet. |
...
| `mars doctor [--repo <path>] [--json]` | Implemented, expanding | Checks Go, config, model registry, models directory, database, llama-server, disk space, anonymous or optional authenticated release access, guardrail/workflow health, mirrored operating-model health, active-plan hygiene, and optional integration configuration. |
...
| `mars release publish-assets` | Retired | T-065 removed the bespoke source publisher. AD-315 now selects a conventional Go/Syft/GitHub-attestation workflow under F-018; target repositories choose their own producer. |
| Conventional source release workflow | Active/tag-only | Builds four source archives plus per-archive SPDX SBOMs and an exact checksum set for an exact repository tag; a separate public-only job creates GitHub keyless provenance after approved visibility. |
...
- MARS source release production follows AD-315's conventional Go/Syft/GitHub-attestation contract and active F-018 plan; generated target repositories choose their own producer and artifact contract
