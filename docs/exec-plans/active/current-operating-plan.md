...
| 10 | T-079 — community, fork-safe CI, Pages, and GitHub controls | F-017-S004 | T-078 source/rehearsal/hosted-CI checkpoints; T-078 sanitation completed before T-080 |
...
- **T-078 — passed 2026-08-25:** AD-315's dormant standard workflow, compatible GitHub-attestation consumer, and two-root no-publish rehearsal pass through exact source `d411cbe`; exact pushed sources `a4bbf81` and `2dd73fc` plus hosted runs `32848968969` and `32849610523` pass Go 1.25.13, Go 1.27.0, dependency notices, the intentional Go 1.25.12 rejection, and zero called application vulnerabilities without an exception. After fresh owner approval and fail-closed revalidation, the exact hosted transaction deleted 500 legacy assets, 77 deployments, and 474 sealed completed runs; preserved 56 Releases, 301 tags, and 33 newer runs; and enabled future-only immutable Releases. Closure source `2d5c05e` passed hosted run `32907186559`. The receipt is `docs/validation/reports/2026-08-24-t078-hosted-state-revalidation.md`.
- **T-079 — passed 2026-08-25:** Source `3502110` and hosted run `32903803262` pass. GitHub enforces GitHub-owned full-SHA Actions, approval-gated read-only/no-secret private-fork workflows, Dependabot alerts/security updates, authenticated bot DCO handling, Discussions, and exact active ruleset `21491158`; visibility remains private and Releases/tags/Pages/Apps are unchanged. The exact receipt is `docs/validation/reports/2026-08-25-t079-private-contribution-controls.md`. Public-only CodeQL, secret scanning/push protection, private vulnerability reporting, Pages, and the live hostile-fork smoke remain T-080 work after visibility.
