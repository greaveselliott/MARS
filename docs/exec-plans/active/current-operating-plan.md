...
- **Current Primary Blocker:** The AD-315 source and two-clean-root no-publish rehearsal pass through exact source `d411cbe`. On 2026-08-24, the hosted seal bound 56 Releases, 500 assets, 473 completed runs, 77 deployments, zero artifacts/caches, and disabled immutable Releases. The exact deletion/setting transaction is not yet approved, and hosted source compatibility is red on six called Go 1.25.12 vulnerabilities fixed in 1.25.13. Public visibility, launch attestations, releases, and announcement remain later gates.
- **Next Primary Action:** Present the exact 500-asset, 77-deployment, 473-run cleanup plus future-only immutable-Release transaction for separate approval. Stop on any live-state drift; do not broaden the vulnerability disposition or expand the deferred bespoke platform.
...
- **T-078 — current and blocked:** AD-315's dormant standard workflow, compatible GitHub-attestation consumer, and two-root no-publish rehearsal pass through exact source `d411cbe`. The current 473-run hosted seal and exact 500-asset/77-deployment/473-run cleanup manifests pass read-only reconciliation, but destructive cleanup and future-only immutable Releases still need separate approval. Hosted CI is red only in the Go 1.25.12 compatibility lane on six called standard-library vulnerabilities fixed in 1.25.13; no exception is broadened.
