...
- **Current Primary Blocker:** The AD-315 source and two-clean-root no-publish rehearsal pass through exact source `d411cbe`. On 2026-08-25 the owner approved the bounded Go 1.25.13 source-floor remediation after hosted Go 1.25.12 reported six called standard-library vulnerabilities; hosted source compatibility must pass the Go 1.25.13 and intentional Go 1.25.12 rejection lanes without an exception. The revised hosted cleanup/immutable-Release transaction remains separately approval-gated after deployment-state drift. Public visibility, launch attestations, releases, and announcement remain later gates.
- **Next Primary Action:** Push the approved Go 1.25.13 source-floor checkpoint and require every hosted source-compatibility lane to pass. Keep the revised 500-asset/474-run cleanup plus future-only immutable-Release transaction separate and approval-gated; do not broaden the vulnerability disposition or expand the deferred bespoke platform.
