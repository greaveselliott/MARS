...
- **Current Primary Blocker:** The AD-315 source and two-clean-root no-publish rehearsal pass through exact source `d411cbe`. The hosted object seal is stale and no deletion or future-only immutable-Release mutation has separately approved exact authority. Public visibility, launch attestations, releases, and announcement remain later gates.
- **Next Primary Action:** Reacquire and scan the complete current hosted surface, freeze exact IDs and consequences, and request separate owner approval for the sanitation/future-only immutable-Release transaction. Do not activate the dormant workflow or expand the deferred bespoke platform.
...
- **T-078 — current and blocked:** AD-315's dormant standard workflow, compatible GitHub-attestation consumer, and two-root no-publish rehearsal pass through exact source `d411cbe`. Hosted CI, the current hosted seal, and a separately authorized sanitation/future-only immutable-Release transaction remain incomplete. The exact 65-run post-T-072 delta frozen at 466 runs is historical evidence only; every later run must be acquired/scanned/resealed before deletion.
