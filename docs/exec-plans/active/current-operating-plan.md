...
**Success Evidence:** The 2026-05-20 `demo-123-run11` replay reached product planning, ticketing, Engineer completion, QA, Security, Dogfood, and local release notes with zero intervention-debt tickets; release-blocked publication stopped dispatch without remote mutation or a Dogfood loop. The 2026-05-20 `demo-api-run1`, `demo-api-run2`, `demo-api-run4`, `demo-api-run5`, `demo-api-run6`, `demo-api-run7`, and `demo-api-run8` non-static replays reached product-specific planning, ticketing, and partial Go API implementation without intervention-debt ticket amplification.
...
**Current Failing Scenario:** As of 2026-05-20, `demo-api-run8` confirmed repo-local `go build -o` outputs are blocked before creating binaries and the target stays clean, then exposed malformed bare-port validation: Engineer called `shell_exec` with `argv:[":8080"]` twice after the build block and ended with `circle_detected`.
**Walking Skeleton Slice:** Make `shell_exec` reject bare port tokens such as `:8080` before process execution, tell roles to start the real server command with `background:true` and probe with curl, mirror the rule into generated Engineer guidance, rerun the API canary, then target the next largest matrix-backed turn sink.
