...
**Success Evidence:** The 2026-05-20 `demo-123-run11` replay reached product planning, ticketing, Engineer completion, QA, Security, Dogfood, and local release notes with zero intervention-debt tickets; release-blocked publication stopped dispatch without remote mutation or a Dogfood loop. The 2026-05-20 `demo-api-run1`, `demo-api-run2`, `demo-api-run4`, `demo-api-run5`, `demo-api-run6`, `demo-api-run7`, `demo-api-run8`, `demo-api-run9`, and `demo-api-run10` non-static replays reached product-specific planning and ticketing without intervention-debt ticket amplification; `demo-api-run9` reached QA, Security, Dogfood, local release notes, and quality-score export.
...
**Current Failing Scenario:** As of 2026-05-20, `demo-api-run10` confirmed scratch-validation prevention code can reach Engineer implementation and tests, then exposed a stale `go run` child server from prior background cleanup occupying port `8080` and causing repeated port-token recovery attempts.
**Walking Skeleton Slice:** Make `shell_exec` background cleanup kill known descendants as well as the tracked process group, rerun the API canary from a clean target, confirm stale server children no longer leak across jobs, then target the next largest matrix-backed turn sink.
