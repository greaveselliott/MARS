...
- **T-078 — passed 2026-08-25:** AD-315's dormant standard workflow, compatible GitHub-attestation consumer, and two-root no-publish rehearsal pass through exact source `d411cbe`; exact pushed sources `a4bbf81` and `2dd73fc` plus hosted runs `32848968969` and `32849610523` pass Go 1.25.13, Go 1.27.0, dependency notices, the intentional Go 1.25.12 rejection, and zero called application vulnerabilities without an exception. After fresh owner approval and fail-closed revalidation, the exact hosted transaction deleted 500 legacy assets, 77 deployments, and 474 sealed completed runs; preserved 56 Releases, 301 tags, and 33 newer runs; and enabled future-only immutable Releases. The receipt is `docs/validation/reports/2026-08-24-t078-hosted-state-revalidation.md`.
