...
**Current Failing Scenario:** F-017-S003 — exact source `6c044c1` implements and tests the dormant AD-315 workflow, conventional producer, compatible GitHub-attestation consumer, Go 1.27 migration, and zero-called vulnerability baseline. The two-clean-root no-publish rehearsal, current hosted-state seal, and separately authorized sanitation/immutable-Release transaction remain incomplete. The rejected GoReleaser/Cosign and bespoke-containment work is preserved as a non-authorizing checkpoint. The owner reports GitHub funding restored; hosted CI still needs a successful rerun.
...
- **Current Primary Blocker:** The AD-315 source implementation is pushed at `6c044c1`, but its two-clean-root no-publish rehearsal has not run. The hosted object seal is stale and no deletion or immutable-Release mutation has separately approved exact authority. Public visibility, launch attestations, releases, and announcement remain later gates.
- **Next Primary Action:** Run the conventional producer twice from distinct clean roots without credentials or hosted mutation, verify both nine-file unsigned distributions and equivalent SBOM semantics, and exercise the standard public attestation fixture. Stop on a concrete upstream/tooling blocker rather than expanding the deferred bespoke platform.
...
| AD-315 can produce and attest the exact ten-asset release without bespoke infrastructure | Standard Go, Syft, and GitHub attestation mechanisms are supported; repo integration is not implemented | 0.70 | T-078 dormant workflow, consumer, two-root no-publish rehearsal, and hosted CI |
...
- **T-078 — current and blocked:** AD-315 is selected and documented. The dormant standard workflow, compatible GitHub-attestation consumer, two-root no-publish rehearsal, coverage/docs gates, hosted CI, current hosted seal, and separately authorized sanitation/immutable-Release transaction remain incomplete. The exact 65-run post-T-072 delta frozen at 466 runs is historical evidence only; every later run must be acquired/scanned/resealed before deletion.
