...
**Success Evidence:** The 2026-05-20 `demo-123-run11` replay reached product planning, ticketing, Engineer completion, QA, Security, Dogfood, and local release notes with zero intervention-debt tickets; release-blocked publication stopped dispatch without remote mutation or a Dogfood loop. The 2026-05-20 `demo-api-run1`, `demo-api-run2`, `demo-api-run4`, `demo-api-run5`, `demo-api-run6`, `demo-api-run7`, `demo-api-run8`, and `demo-api-run9` non-static replays reached product-specific planning and ticketing without intervention-debt ticket amplification; `demo-api-run9` reached QA, Security, Dogfood, local release notes, and quality-score export.
...
**Current Failing Scenario:** As of 2026-05-20, `demo-api-run9` confirmed bare-port rejection works and the API lifecycle reaches local release notes, then exposed scratch validation pollution: Engineer committed a root `validate.sh` after forbidden cleanup attempts, and Dogfood proved the script depends on non-portable `timeout`.
**Walking Skeleton Slice:** Block new repo-root validation shell scripts, reject external `timeout`/`gtimeout` commands in `shell_exec`, mirror the rule into generated Engineer and Dogfood guidance, rerun the API canary, then target the next largest matrix-backed turn sink.
...
| F-007-S011 | Passing | `go test ./internal/tools -run 'TestShellExec(BlocksGoBuildOutputInsideRepoBeforeArtifact|AllowsGoBuildOutputOutsideRepo|MalformedArgsNotMaskedByDirtyArtifact)'` covers pre-execution blocking for explicit `go build -o` outputs inside the target repo and malformed shell payload surfacing; `demo-api-run8` confirmed no `task-notes-api` binary was created and the target stayed clean after quality-evidence commit. |
