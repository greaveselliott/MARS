...
- **F-017-S004:** T-079 is the current independent private slice for community files, fork-safe CI, DCO/CODEOWNERS, governance/support, private-safe GitHub settings, rulesets, Issues, and Discussions. Account-wide GitHub App administration is excluded unless a MARS workflow has an actual dependency. Because this private personal repository cannot production-enable CodeQL/code scanning, secret scanning/push protection, private vulnerability reporting, Pages, or a genuine hostile public-fork smoke without unavailability or premature publication, T-079 stages that public-only bundle and live fork smoke for immediate post-visibility enablement and verification in T-080.
