...
- **F-017-S004:** T-079's private slice passes its source and live-setting boundary at source `b807afa` and hosted run `32901437495`: community files, fork-safe read-only/no-secret CI, DCO/CODEOWNERS, governance/support, Dependabot, Discussions, GitHub-owned full-SHA Actions, and exact active ruleset `21491158` are installed while visibility remains private. Final evidence-source hosted verification is pending. Account-wide GitHub App administration is excluded unless a MARS workflow has an actual dependency. CodeQL/code scanning, secret scanning/push protection, private vulnerability reporting, Pages, and a genuine hostile public-fork smoke remain T-080 work after the separately approved visibility change.
