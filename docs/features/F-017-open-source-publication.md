...
- **F-017-S001:** The proposed private publication surface is reconstructed. T-070 froze 302 canonical publication refs and scanned all 11,954 reachable objects with both exact pinned tools without accepted-scan errors/skips. Elliott classified Gitleaks group `a2a292e31d652f22` as synthetic test stubs after owner-only review; URI group `e32927624f4a2cac` remains unclassified before any required rotation/revocation and a complete rescan. GitHub-hosted content review, manual privacy/IP/provenance/name review, and committed owner disposition remain blocked subsequent slices. Local stashes, administrative refs, unreachable objects, and the unrelated Codex worktree are preserved non-publication exceptions rather than inputs to T-070.
