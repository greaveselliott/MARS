...
- **F-018-S002:** In progress under T-066. The ticket was created through `ticket_create` with an exact offline Sigstore-bundle/checksum/archive contract, full workflow-SHA-to-binary-commit binding, bounded hostile extraction, atomic preservation, and six planned separate commit checkpoints after splitting signature/checksum verification from archive extraction. No implementation or real MARS signing/publication evidence exists yet.
