...
- **F-018-S002:** In progress under T-066 with only checkpoint E remaining. A1 is pushed at `fcf7397`: exact offline Sigstore trust, workflow/repository/tag/full-commit identity, transparency evidence, raw checksum bytes, and the canonical eight-entry checksum contract verify with fixed redacted failures. A2 is pushed at `b824b91`: the authenticated checksum result is identity-bound, the canonical platform archive is digest-verified before parsing, exact bounded archive structure is enforced, and the contained binary's release build metadata is verified before its cloned bytes are returned. B1 is pushed at `f3ed495`: exact immutable Release/tag/commit/inventory acquisition is anonymous-first and credential-safe, all eight signed artifact digests bind to remote metadata, replay/drift/quotas fail closed, and only a cloned A1/A2-verified candidate is returned without filesystem mutation. B2 is pushed at `92d7ddd`: a network-free, descriptor-bound transaction serializes updates, durably stages and verifies the candidate and backup, atomically replaces the fixed binary, and either proves the installed result, proves compensation, or retains explicit recovery-required evidence without overwriting an unknown entry. B3 is pushed at `683daf8`: release-mode `Run` binds implicit latest to the exact running executable and digest, performs B1 then B2 before PATH repair, rejects destination drift under the B2 lock without transaction state, preserves source/main and authority-free dry-run behavior, and removes raw release URLs from live plans/output. C is pushed at `f45d5d2`: the circularly untrustworthy shell binary bootstrap is retired to a non-mutating, nonzero-exit reviewed-source route, with hostile-environment tests proving no network, credentials, commands, temp files, privilege, or destination mutation. D is pushed through `64c7d3b`: standalone verifier/audit code and commands, raw/checksum-only helpers, MarsCLI references, and stale generated/live guidance are retired; source MARS now states the signed consumer contract and targets own their verifier. Focused release/selfupdate/CLI, tools/scanner, positive Sigstore/archive/acquisition/replacement, docsconsistency, and DocSync gates passed; CTO-weekly, QA, Security, and Orchestrator are GO. Checkpoint E's installed/offline lifecycle evidence remains incomplete, and fresh packaged bootstrap remains unavailable pending a non-circular trust mechanism; the repository remains private, Primary Status remains `primary_blocked`, and no real MARS signing or publication evidence exists.
- **F-018-S003:** Pending T-068 after T-066 completes.
