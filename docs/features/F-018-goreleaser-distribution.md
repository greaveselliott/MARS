...
- **F-018-S003:** Current under T-068; the ticket exists and its private rehearsal has not run. Its contract separates clean source setup, direct native execution of unsigned snapshots, and offline synthetic/preverified updater fixtures. The read-only snapshot workflow must succeed with exact publication-disabled skip flags; it has no draft/release job and no live publication failure is invoked. No completion evidence exists yet.
