...
- **F-018-S002:** Blocked at T-066 checkpoint A1 while owner-approved T-067 establishes the source compatibility prerequisite. Official `sigstore-go v0.7.0` was rejected with 14 called findings including `GO-2026-5952`. Newest fixed `v1.2.2` compiles the exact offline wrapper; its declared Go 1.25.8 floor still exposes eight called standard-library findings, while the identical path on Go 1.25.12 reports zero called findings. T-067 therefore owns `go 1.25.12`, retained release `toolchain go1.26.5`, exact minimum-version CI, and packaged operation without an externally installed Go toolchain before T-066 resumes. No verifier dependency, real MARS signing, or publication evidence exists yet.
