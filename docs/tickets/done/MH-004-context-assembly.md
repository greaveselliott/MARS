...
- [x] Assembler produces a system prompt containing role prompt, guardrails, knowledge routes, and trigger context
- [x] Only in-scope guardrails are included (scope filtering works)
- [x] Knowledge routes are formatted as "when working on X, read Y" guidance
- [x] Section headers are present and clear

### Edge cases and negative paths
- [x] Context budget exceeded → lower-priority sections truncated, role prompt never truncated
- [x] Missing role prompt file → descriptive error
- [x] No guardrails in scope → section omitted (not empty section header)
- [x] No knowledge routes configured → section omitted
- [x] Empty trigger context → section omitted
...
- [x] Test verifies scoping: engineer role gets engineering guardrails, not security guardrails
- [x] Test verifies budget enforcement: with a low budget, sections are truncated in correct order
- [x] context-efficiency.md updated with discoveries
