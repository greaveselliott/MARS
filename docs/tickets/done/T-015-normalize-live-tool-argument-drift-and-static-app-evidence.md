...
## Context
Run 11 reached terminal release-blocked dispatch, but the live trace showed tool payload drift and static app evidence waste still consuming turns. Release Manager emitted mars_cli args as a string list, shell_exec argv as a Python-style list string, Engineer left static assets pointing at a non-existent duplicate F-001 feature path, and Dogfood spent turns discovering src/index.html.

## Requirements
- mars_cli accepts list-shaped args emitted as strings.
...
## Acceptance Criteria
- Unit tests cover mars_cli args strings and shell_exec Python-list argv strings.
...
- Generic list-shaped argument normalization was completed for
  `mars_cli.args`, `shell_exec.argv`, `workspace_hygiene.paths`,
