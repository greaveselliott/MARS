---
id: MH-009
title: "Implement mars run: CLI, bundle reader, terminal trace, first demo"
priority: high
complexity: large
...
# MH-009: Implement `mars run` — CLI, bundle reader, terminal trace, first demo

## Context

This is the first visible, demonstrable milestone. A user runs `mars run pipeline-fixer --repo /path` and sees a role execute against a real repo with the full trace streaming in the terminal.

Reference: delivery schedule M3

## Requirements

### CLI framework (`cmd/mars/main.go`)
- Replace placeholder with cobra (or equivalent) CLI framework
- `run` subcommand: positional `<role>` with `mars run <role> --repo <path>`; flags `--model-endpoint` (optional override), `--trace` (verbosity), `--dry-run`
- `version` subcommand
...
- [x] `mars run pipeline-fixer --repo /path` executes and produces visible output
...
### Edge cases
- [x] Missing `.harness/` directory → actionable error ("Run mars init to create one")
...
### Observability
- [x] Demo recorded: terminal session showing mars run against a repo
