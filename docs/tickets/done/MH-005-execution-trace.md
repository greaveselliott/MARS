...
- [ ] Recorder captures all turns of a multi-turn conversation
- [ ] Each turn includes: role, content, tool calls (if any), tool results (if any), timestamp, token estimate
- [ ] Summary correctly computes: total duration, total tokens, tool list, turn count, outcome
- [ ] Trace persists to SQLite and can be retrieved by job_id
- [ ] File export writes valid JSON Lines

### Edge cases and negative paths
- [ ] Very long tool result (>100KB) is truncated in the trace with a note
- [ ] SQLite write failure returns error, does not crash the job
- [ ] Retrieving a non-existent trace returns nil, not error
...
- [ ] Test records a 5-turn conversation and verifies summary accuracy
- [ ] Test verifies SQLite round-trip (write and read back)
