...
bdd_scenarios: []
end_to_end_evidence: "go run ./cmd/mars run engineer --repo /path/to/local-redacted --dry-run --trace --no-init"
evidence_links: ["docs/validation/reports/2026-05-19-mars-observer-validation.md"]
verified_by: "go test ./cmd/mars -run 'TestRunCommand(NoInit|AutoInit|RejectsRepoLocalLogFile)|TestMarsCLI'; go run ./cmd/mars run engineer --repo /path/to/local-redacted --dry-run --trace --no-init; git -C /path/to/local-redacted status --short --branch"
owner: "Codex"
