...
blocked_by: []
trace_id: "TBD"
next_action: "Rerun the non-static API canary after no-op shell guidance, confirm Engineer stops the validation PID and completes the ticket lifecycle, then continue to the next turn sink."
...
- Rerun the API canary to confirm no-op shell guidance lets Engineer stop the
  validation PID and complete the ticket lifecycle, then continue checking
  scratch-validation and timeout rejection in the live lifecycle.
