...
- [ ] Orchestrator surveys can create or update prioritized jobs for unattended
      failure states.
- [ ] Queue ownership prevents duplicate work on the same eligible ticket.
- [ ] Telemetry triage runs during surveys, not only after job completion.
- [ ] Payload modes are represented in jobs and prompts.
- [ ] Concurrency groups and daily caps constrain repeated retries.

### Edge cases and negative paths

- [ ] Stuck-job detection does not interrupt healthy long-running jobs.
- [ ] No-op runs are detected without relying on GitHub or PR state.
- [ ] Optional GitHub events enrich routing but are not required for local
      orchestration.

### Observability, docs, and regressions

- [ ] Integration tests cover survey-to-job routing for stale tickets and failed
      checks.
- [ ] Trace and score records show why Orchestrator created each job.
- [ ] Dashboard and doctor surfaces point to the same source signals.
