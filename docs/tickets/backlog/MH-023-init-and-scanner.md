...
- `init` writes `.harness/` skeleton: `manifest.yaml`, sample roles, guardrails stub, knowledge routes placeholder
- Create `docs/tickets/backlog/` with seed tickets if missing; mirror exec-plans folders; append or create `AGENTS.md` section for harness conventions (non-destructive merge)
...
- [ ] Scanner on repo with untested package generates markdown tickets with YAML frontmatter consistent with repo ticket format
